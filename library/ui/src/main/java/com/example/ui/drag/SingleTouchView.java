...
public class SingleTouchView extends View {
    ...
    // 旋转图片默认宽高
    public static final int DEFAULT_DRAWABLE_WIDTH = 100;
    public static final int DEFAULT_DRAWABLE_HEIGHT = 100;
    // 控制图标默认宽高
    public static final int DEFAULT_CONTROL_DRAWABLE_WIDTH = 25;
    public static final int DEFAULT_CONTROL_DRAWABLE_HEIGHT = 25;
     ...
     * 控制图标默认宽高
     */
    private int controlDrawableWidth = DEFAULT_CONTROL_DRAWABLE_WIDTH;
    private int controlDrawableHeight = DEFAULT_CONTROL_DRAWABLE_HEIGHT;

    /**
     * 是否移除控制图标
     */
    private boolean isRemoveControlDrawable = false;

    /**
     * 海报编辑区域宽高
     */
    private int editAreaWidth, editAreaHeight;
    ...
    private void obtainStyledAttributes(AttributeSet attrs) {
        ...
        Drawable sourceDrawable = mTypedArray.getDrawable(R.styleable.SingleTouchView_source);
        mBitmap = drawable2Bitmap(sourceDrawable);
        ...
        controlDrawableWidth = mTypedArray.getDimensionPixelSize(
                R.styleable.SingleTouchView_controlDrawableWidth, DEFAULT_CONTROL_DRAWABLE_WIDTH);
        controlDrawableHeight = mTypedArray.getDimensionPixelSize(
                R.styleable.SingleTouchView_controlDrawableHeight, DEFAULT_CONTROL_DRAWABLE_HEIGHT);
    ...
    private void init() {
        ...
        mControlDrawableWidth = controlDrawableWidth;
        mControlDrawableHeight = controlDrawableHeight;

        DisplayMetrics dm = context.getResources().getDisplayMetrics();
        editAreaWidth = dm.widthPixels;
        // dp2px(context, 60)为底部导航高度
        editAreaHeight = dm.heightPixels - dp2px(context, 60);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (null != mViewGroup) {
            ...
            if (0 == parentWidth || dp2px(context, 100) == parentWidth
                    || 0 == parentHeight || dp2px(context, 100) == parentHeight) {
                mCenterPoint.set(editAreaWidth / 2, editAreaHeight / 3);
            } else {
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        if (specMode == MeasureSpec.EXACTLY) {
            ...
            result = dp2px(context, 100);
            if (specMode == MeasureSpec.AT_MOST) {
                // 如果是AT_MOST模式，测量宽度就是你自己设置的默认宽度与父类传递过来的宽度之间的一个最小值
                //（为什么取最小值呢，
                // 因为如果你设置的默认值很大，已经超过了父控件的大小，那么这个控件在父控件里就显示不全了，
    ...
    private Bitmap drawable2Bitmap(Drawable drawable) {
        try {
            ...
            Bitmap bitmap = Bitmap.createBitmap(
                    intrinsicWidth <= 0 ? DEFAULT_DRAWABLE_WIDTH : intrinsicWidth,
                    intrinsicHeight <= 0 ? DEFAULT_DRAWABLE_HEIGHT : intrinsicHeight,
                    Config.ARGB_8888);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 处于可编辑状态才画边框和控制图标
        if (isEditable && !isRemoveControlDrawable) {
            mPath.reset();
            ...
            // 画旋转, 缩放图标
            if (null != controlDrawable) {
                controlDrawable.setBounds(mControlPoint.x - mControlDrawableWidth / 2, mControlPoint.y - mControlDrawableHeight / 2,
                        mControlPoint.x + mControlDrawableWidth / 2, mControlPoint.y + mControlDrawableHeight / 2);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mStatus == STATUS_ROTATE_ZOOM) {
                    ...
                    // 图片某个点到图片中心的距离
                    float bitmapToCenterDistance = (float) Math.sqrt(
                    ...
                    PointF centerToProMove = new PointF(
                            (mPreMovePointF.x - mCenterPoint.x), (mPreMovePointF.y - mCenterPoint.y));

                    // center -> curMove 的向量
                    PointF centerToCurMove = new PointF(
                            (mCurMovePointF.x - mCenterPoint.x), (mCurMovePointF.y - mCenterPoint.y));
     ...
     * 设置控制图标
     */
    public void setControlDrawable(Context context, Drawable drawable,
                                   int controlDrawableWidth, int controlDrawableHeight) {
        this.controlDrawable = drawable;
        mControlDrawableWidth = dp2px(context, controlDrawableWidth);
        mControlDrawableHeight = dp2px(context, controlDrawableHeight);
        transformDraw();
    }
    ...
    /**
     * 不显示边框, 需要在 setImageBitmap 方法之前执行
     */
    ...
    public boolean isRemoveControlDrawable() {
        return isRemoveControlDrawable;
    }

    /**
     * 设置是否移除 控制图标
     */
    public void setRemoveControlDrawable(boolean isRemoveControlDrawable) {
        this.isRemoveControlDrawable = isRemoveControlDrawable;
        invalidate();
    }
